...
def forward_backward(obs, states, start_p, trans_m, emit_m):
    ...
    forward_probs = []
    # initialization, when it's in timeline 0, P(State_0 | Observation_0:0) can be the prior probability of the state
    f_0_to_pre_t = dict((s, emit_m[s][obs[0]] * start_p[s]) for s in states)
    forward_probs.append(f_0_to_pre_t)
    ...
    backward_probs = []
    # initialization here, i = T(the length of the sequence and exceed the max index t-1), 
    # and then Observation_t:t-1 becomes a null sequence, its probability is 1
    b_post_k_to_t = dict((s, 1.0) for s in states)
    backward_probs.insert(0, b_post_k_to_t)
